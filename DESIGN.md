...
- Boxed elements, Groups, and multi-selection use a straight solid-blue ring separated from painted bounds by a 6px screen-space gap, with eight square resize handles plus a rotate connector/handle. A singly selected Line or two-point Arrow instead shows two square endpoint handles plus one circular midpoint bend handle, while Polyline shows only one square handle per persisted point; line-like editing does not draw a rectangular bounds ring. Marquee, restrained snap guides, handle size, hit regions, stroke, and spacing stay screen-space stable, and overlay paint itself ignores pointer events.
- A singly selected Text element shows left/right width handles, four corner size handles, and rotate, but no standalone top/bottom handles. Width drag changes persistent wrapping width without changing font size; corner drag always scales persistent font size proportionally and scales fixed wrapping width with it. The corner uses the closest point on the original proportional resize axis instead of letting the shorter Text dimension dominate and fling the handle away from the pointer. Auto-width remains auto-width for corner scaling, and text glyphs are never stretched by a resize affine.
- Selection handles are keyboard and screen-reader controls, not decorative paint. Tab focuses each named handle with a visible ring; Enter/Space grabs and commits, arrows adjust by 1 screen pixel, Shift+Arrow adjusts by 10 screen pixels while preserving Rust constraints, and Escape cancels. Grab, preview, commit, and cancel state is announced without requiring a framework-specific host.
- Dragging a Line or Arrow endpoint, or any Polyline vertex, previews resolved geometry in Rust and commits one `update_path_points` Transaction on release. Dragging the circular midpoint handle creates or updates one quadratic curve and commits one `update_path_curve` Transaction; returning it to the chord midpoint restores a straight path. `Shift` constrains an edited endpoint segment to 45° increments. Double-clicking within 10px screen-space of a selected Polyline segment inserts a projected vertex; deleting a selected Polyline vertex keeps the three-point minimum. Dragging the path body still moves the whole element.
...
_Last updated: 2026-07-23 | Reason: define semantic Text resizing and accessible selection handles_
