...
第二页面只读打开并可显式请求接管；当前 writer 只有在最新 revision 安全保存后才释放 Web Lock，
新页面必须重载 verified stable snapshot 并重新取得真实 lease，旧页面则保持只读。
...
矩形、基础图形、自由笔和文本的有限样式预设也已接入同一条 Rust Command/Transaction 路径：选择时显示
共享的上下文 Style 面板；相同预设是 no-op，实际变化可 Undo/Redo 并随本地文档恢复，三种宿主
不各自保存样式副本。线类与封闭图形使用元素级 `S/M/L/XL` Size，默认 `M`，由 Rust 统一解析为
2/4/6/8px 描边；Arrow 的箭身与箭头尺寸随 Size 成套变化。产品不再把 Clean/Sketch 暴露为整板主题；
Rust 暂时保留旧 Render Profile 与确定性 Sketch resolver，作为旧快照和测试的内部兼容边界。
Phase 1B 的第一条编辑器基础切片进一步加入框选/多选、八向缩放与旋转、嵌套 Group、同层级
顺序、内部剪贴板、六向对齐，以及带实际 Guide 的移动吸附。Document 当前已迁移到 Schema V6：
...
吸附到绝对 45° 倍数。
基础图形切片在此之上加入 Ellipse、Diamond、Line、Polyline 与 Arrow。五种图形都是
Schema V6 中的 Rust 持久语义，而不是 SVG path 的 UI 别名。工具栏点击只进入创建工具：
Rectangle/Ellipse/Diamond/Line/Arrow 通过拖拽创建，Polyline 通过逐点点击并用双击或 `Enter`
完成；创建期间只有 Rust 瞬态 Scene preview，有效完成后才形成一个 Command/Transaction、
选中新元素并回到 Select。框形支持 `Shift` 等边与 `Alt` 中心展开，Line/Arrow/Polyline 新线段
支持 `Shift` 45° 约束，微小拖拽不创建。完成后统一复用现有选择、样式、移动、缩放、旋转、
Group、剪贴板、对齐、吸附与 Undo/Redo。
Ellipse/Diamond 提供填充与描边，Line/Polyline/Arrow 提供描边；普通 Arrow 仍是自由几何，
不提前引入 Connector 的节点绑定和自动路由。Rust 将这些语义确定性解析成通用 Scene path，
React、Vue、Vanilla 以及 SVG Renderer 不各自实现图形算法。
完成后的 Line、Polyline 与 Arrow 单选时显示 screen-space 稳定的顶点手柄，而不是八向缩放手柄。
拖动端点/顶点会先由 Rust 解析瞬态 preview，再在释放时形成一个可 Undo 的几何 Transaction；
`Shift` 约束相邻线段到 45° 增量。选中 Polyline 后可双击线段插入投影顶点，并用
`Delete`/`Backspace` 删除已选顶点（至少保留三个点）；拖动线条本体仍移动整个图形。
两点 Line/Arrow 还显示一个圆形中点手柄；拖动它会在 Rust 中创建单段 quadratic curve，
实时 Scene、视觉 bounds、hit-test 与最终 Document 共用同一几何。Arrow 箭头跟随终点切线，
Size 决定的箭头长度与开口不会因弯曲角度缩水；一次拖动只提交一个可 Undo/Redo 的
`update_path_curve` Transaction。Polyline 和未来 Connector routing 不复用这套曲线字段。
...
- [Phase 1B 基础图形](docs/planning/phase1b-basic-shapes.md)
- [Phase 1B 形状直接创建](docs/planning/phase1b-shape-creation.md)
- [Phase 1B Line/Arrow 曲线编辑](docs/planning/phase1b-curved-path-editing.md)
- [Phase 1B 显式编辑权接管](docs/planning/phase1b-lease-takeover.md)
