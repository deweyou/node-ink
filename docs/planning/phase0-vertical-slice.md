...
> 日期：2026-07-21
> 状态：已实现并验证
> 覆盖 Spike：S0、S1、S2、S3、S4、S5、S6、S7、S8、S9、S10、S11、S12
...
| S9 two-tab Web Locks fixture | writer/contender、显式释放、关闭 writer 后接管、无锁只读、并发 revision conflict 全部通过 |
| S10 Diagram Operation browser fixture | create/move/update/delete × 5；dry-run 不变更 r0，apply 一次到 r1，失败原子拒绝，重放一致，Undo 到 r2 |
| S11 host lifecycle browser fixture | 25/25 create/move/undo；100 add/remove listener；25 handle dispose；React/Vanilla 可见闭环一致；0 残留 SVG |
| S12 repeatable toolchain fixture | 首次/增量/缺失重建 hash 一致；Cargo 2.67s→0.03s；rustc failure exit 1；优化 WASM 双宿主运行 |
| `git diff --check` | 通过 |
...
两标签页真实浏览器证据见 [`phase0-s9-web-locks.json`](../benchmarks/phase0-s9-web-locks.json)，测试源代码
锚定提交 `fb5c4e0`。同一 `documentId` 上先后验证 writer、竞争者、显式释放、标签页关闭和重新接管；
另用同一 IndexedDB head 并发提交验证 expected revision。

- 首标签页 0.4ms 获取 writer；第二标签页 5.3ms 内返回 `readonly/held_elsewhere`，没有等待或抢写。
- 显式 release 为 0.1ms，随后接管 0.3ms；直接关闭 writer 标签页后，另一标签页 0.2ms 获取 writer。
- Web Locks 不可用/请求失败时一律 `readonly/unsupported`；Phase 1A 不实现不可靠的自制过期 lease。
- 两个并发 expected-revision 写入始终一个成功、一个 `revision_conflict`，恢复 revision 为 1，silent last-write-wins 为 false。
- 接管流程必须先加载 S7 verified stable snapshot，再创建新的内存 Engine；lease 本身不授权复用旧状态。

## S10 Diagram Operation 原子事务决策

跨 Rust/WASM/Web 的真实浏览器证据见 [`phase0-s10-diagram-operation.json`](../benchmarks/phase0-s10-diagram-operation.json)，
测试源代码锚定提交 `c49e090`。单批依次执行 create A、create B、move A、resize A、delete B，覆盖 V1 的全部最小操作。

- dry-run 完成 5 个 planned 结果与 1 个新增 Scene node preview，但 Document/revision/history 保持 r0；本机耗时 0.1ms。
- apply 的 5 个结果一次提交到 r1，只留下 `rect-a`；Renderer 直接应用来源无关的 ScenePatch，DOM 中只出现 1 个 Scene node。
- 相同 batch 在同起始 revision 的第二个 Engine 上重放，结果与最终 Scene 逐字节一致；在 r1 上再次提交明确 revision conflict 并保持 r1。
- “先创建、后移动不存在元素”的失败批次保持 r0/0 elements；成功批次只产生一个 Undo entry，Undo 后为空文档 r2。
- Web 行/分支覆盖为 99.00%/92.08%；Rust 总行覆盖 98.86%，新增 operation 模块行/函数覆盖 99.54%/100%。
- 本项是语义 fixture；因已记录的 macOS provenance 问题，Rust 使用 release profile、临时关闭 wasm-pack 内置 wasm-opt。可重复优化构建由 S12 单独修复与验证，不把本项耗时当性能门槛。

## S11 框架无关宿主生命周期决策

真实浏览器与单测证据见 [`phase0-s11-host-lifecycle.json`](../benchmarks/phase0-s11-host-lifecycle.json)，
测试源代码锚定提交 `e70a684`。React adapter 与 Vanilla host 都使用同一 `createController` 组合真实 WASM Engine、EditorWebController 和 SVG Renderer。

- 两个可见入口逐个点击 Rectangle、Move、Undo，均得到 r1→r2→r3、1 element、矩形 x=80；宿主差异只在 UI adapter。
- 25 个全新 Controller/Engine handle 循环全部完成 create/move/undo；每轮最终 r3、矩形回到 x=80。
- 4 类 Pointer listener 每轮各添加/移除一次，总计 100/100，active=0；subscription 每轮只收到 mount/create/move/undo 4 次通知。
- dispose 重复调用仍只释放 25 个 Engine handle 和 25 个 Renderer；释放后 25/25 handle 调用被拒绝，target 中 0 residual SVG。
- 核心 Web lifecycle 源码没有 timer/RAF 引用；`pnpm check` 新增可执行 framework boundary gate，五个 framework-neutral 包均无 React/Vue import。
- `pnpm exec vp build apps/playground` 在 90ms 内产出 React/Vanilla 双入口；Web 行/分支覆盖保持 99.00%/92.08%。

## S12 Vite+、Cargo 与 WASM 工具链决策

完整数据见 [`phase0-s12-toolchain.json`](../benchmarks/phase0-s12-toolchain.json)，构建修复源代码锚定提交
`26ae541`。根 `.npmrc` 继续固定 npm 官方源，Vite+ 0.2.5 负责任务编排，Cargo 1.96.0 保持 Rust 权威边界。

- wasm-pack 0.15 以 release/`--no-opt` 生成 wasm-bindgen 产物，再由 lockfile 固定的 Binaryen 117 以 `-Oz` 优化到同文件系统临时文件后替换。
- 首次/增量 wall time 为 10.24/5.37s，Cargo release 为 2.67/0.03s；Vite task cache=false 时仍复用 Cargo target，避免第二套大型缓存。
- 两次及完整移走 generated 后重建的 WASM 都是 480,459 bytes，SHA-256 为 `c982d5b4…50d995`；JS 与 d.ts 也逐字节一致。
- 根 build wall time 6.31s、Vite bundle 89ms；dist 744KiB，WASM gzip 186.76kB；React/Vanilla 用新优化产物创建矩形到 r1，无浏览器告警。
- `RUSTC=/usr/bin/false` probe 以 exit 1 暴露 `cargo metadata` 错误，证明 Rust 失败不会被 Web check/build 成功掩盖。
- frozen install、官方 registry、check、134 Web tests、双覆盖率、Rust gate 与根 build 已串行通过。
...
- Binaryen wasm-opt：`117`

macOS provenance 曾让 wasm-pack 内置 wasm-opt 的替换流程触发 `Operation not permitted`。
脚本默认把 Cargo target 放在系统临时目录，并显式拆分 wasm-pack 与 Binaryen 优化步骤；可通过
`NODEINK_CARGO_TARGET_DIR` 覆盖 target。两项调整都不改变 Cargo 作为 Rust 构建真相源的边界。
