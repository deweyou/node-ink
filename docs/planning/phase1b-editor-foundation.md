...
- The overlay exposes eight resize handles and one rotate handle for compatible selections. Resize clamps before zero instead of flipping; `Shift` preserves aspect ratio, `Alt` resizes about center, and `Shift` rotation snaps to 15°.
- Rectangle, Stroke, Group, and mixed multi-selection use full affine composition. Stroke width remains a style width. Text has left/right width handles and corner size handles; no standalone top/bottom text handles.
...
- Pointer up emits at most one Transform Command. `Escape`, `pointercancel`, lost capture, and focus loss restore the pre-gesture document exactly.
...
---
*Last updated: 2026-07-23 | Reason: freeze the Phase 1B direct-manipulation foundation*
