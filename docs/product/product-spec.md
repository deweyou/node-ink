...
用户无需登录即可创建画板，完成绘制、选择、编辑和撤销；内容会可靠保存在本地，刷新或重启浏览器后可以恢复。相同内容在 Clean 与 Sketch 两种外观下保持同一语义，并能通过内部结构化操作创建和修改。
...
#### 场景 B：在 Clean 与 Sketch 之间表达

1. 用户在文档级选择 Render Profile。
2. Semantic Document 不发生结构变化。
3. Scene Resolution 使用相同元素参数与 seed 生成稳定几何。
4. 切换 Profile 是可撤销的文档设置变更。
...
- 顶部：返回、文档标题、保存状态、Render Profile。
...
| 层 | 典型内容 | 是否持久化 | 所有者 |
| --- | --- | --- | --- |
| Document | 元素、顺序、样式、文档级 Profile | 是 | Rust Semantic Document |
