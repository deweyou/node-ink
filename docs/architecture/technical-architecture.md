...
Schema V6 是当前持久格式。V0/V1 补齐旧视觉默认值与兼容 Profile，V2 为每个旧元素补 identity affine，V3/V4 开放基础图形 kind，V5 将数值 strokeWidth copy-on-write 映射为元素级语义 Size，V6 为 Line/Arrow 增加显式 nullable quadratic curve；所有迁移都由 Rust 完成且不修改源 payload。`rootOrder` 只包含根元素，Group 的 `childOrder` 持有直接子节点顺序；每个非根元素必须恰好出现一次，层级必须无环。

### 8.2 元素公共字段（当前实现）

Schema V6 的 Rect、Ellipse、Diamond、Line、Polyline、Arrow、Stroke、Text 与 Group 都持有同一种 affine matrix。元素自身的几何字段保持在 local space；Rust 组合祖先与自身 transform，Scene 只输出最终 world transform。封闭图形持久化 fill/stroke/size，线类图形与 Stroke 持久化 stroke/size，Line/Arrow 可额外持久化单段 quadratic curve，Text 持久化 color/fontSize/fontWeight/textAlign。
...
`Ellipse`、`Diamond` 和线类元素保持独立 kind，避免 Renderer 从 path 反推用户语义。它们由 Rust 解析为通用 `ScenePathV1`，SVG Renderer 只消费已解析几何和数值 strokeWidth。Line 和恰好两个 points 的 Arrow 可持有一个 quadratic control；`null` 表示直线。UI 拖动的是曲线在 `t=0.5` 的实际中点，Rust 通过 `control = 2 × midpoint - chordMidpoint` 还原控制点，回到 chord midpoint 时规范化为 `null`。Polyline 与旧的多点 Arrow 不解释该字段。

Size 在 Rust 中解析为 2/4/6/8 的 Scene stroke；Arrow 还按同一个 Size 解析 28/40/56/72 的箭头长度和 25.2/36/50.4/64.8 的开口宽度。Arrow 的 points 与 curve control 保持 local space，但 Rust 使用最终 world-space 末段方向或 quadratic endpoint tangent 解析箭头，并输出 identity-transform Scene path；visual bounds 与 hit-test 复用同一解析，非等比元素或 Group affine 不会拉伸箭头，箭尖始终位于持久 endpoint。普通 `Arrow` 是自由几何；未来 `Connector` 有 source/target binding、port 和 route，是不同领域对象。
...
stateDiagram-v2
    ...
    state Ready {
        ...
        SelectIdle --> CurveEditing: pointer down on midpoint bend handle
        CurveEditing --> CurveEditing: pointer move / Rust quadratic preview
        CurveEditing --> SelectIdle: pointer up / one commit
        CurveEditing --> SelectIdle: escape / cancel
...
- Line/Arrow 的显式 quadratic curve、实际 midpoint handle、曲线极值 bounds、自适应 hit-test 展开与 Arrow 终点切线共享同一几何模块。
